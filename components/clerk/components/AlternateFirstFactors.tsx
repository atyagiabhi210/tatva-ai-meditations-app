...
function AlternateFirstFactors({
  ...
  const filteredFactors = factors.filter(
    (f) =>
      ...
      f.strategy !== selectedFactor?.strategy
  );
    ...
    return (
      ...
      >
        {factor.strategy === "email_code" ? (
          ...
          <Text style={styles.alternateMethodText}>{factor.strategy}</Text>
        )}
      </Button>
    );
  }

  return <>{filteredFactors.map((factor) => createStrategyButton(factor))}</>;
}
